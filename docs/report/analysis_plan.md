...
**Deviation from analysis:** These files are not saved as hard copies, but rather generated by the scrip in real time.
...
**Deviation from analysis plan:** Not every variable name outlined here is used in the analysis, but changes are explianed in the code. This is for clarity and efficiency's sake. Furthemore, final data files are not saved because they are generated in real time by the script.
